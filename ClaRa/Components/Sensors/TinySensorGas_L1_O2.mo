...
//__________________________________________________________________________//
  ...
  Modelica.Blocks.Interfaces.RealOutput yps_O2 "Fraction O2 re. to dry flow"
...
equation
  if unitOption == 1 then
    yps_O2 = if dryGas then gas.x[N_O2]/(1-gas.x[N_H2O]) else gas.x[N_O2];
  elseif unitOption == 2 then
    yps_O2=if dryGas then 100*gas.x[N_O2]/(1-gas.x[N_H2O]) else 100*gas.x[N_O2];
  else
    yps_O2 = -1;
...
<p>Version means first ClaRa version where the applied change was published.</p>
  ...
  Icon(coordinateSystem(preserveAspectRatio=false, extent={{-60,-100},{60,20}}), graphics={
        ...
        Ellipse(
          ...
          fillColor= {0,131,169},
          fillPattern=FillPattern.Solid,
          visible=not dryGas)}),                    Diagram(coordinateSystem(extent={{-60,-100},{60,20}})));
end TinySensorGas_L1_O2;
