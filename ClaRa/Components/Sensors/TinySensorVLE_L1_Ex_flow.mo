...
equation
  ...
  entropy = fluid.s;
  Ex_flow = inlet.m_flow*(noEvent(actualStream(inlet.h_outflow))-ambience.h-ambience.T*(fluid.s-ambience.s));
  H_flow = inlet.m_flow*noEvent(actualStream(inlet.h_outflow));
...
<p>Version means first ClaRa version where the applied change was published.</p>
    ...
    Icon(coordinateSystem(preserveAspectRatio=false, extent={{-60,-60},{60,60}}),
          ...
          lineColor=DynamicSelect({230, 230, 230},  if Ex_flow > 0 then {0,131,169} else {167,25,48}),
        ...
        Line(points={{60,40},{38,40}}, color={27,36,42})}), Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-60,-60},{60,60}}),
                        graphics));
end TinySensorVLE_L1_Ex_flow;
