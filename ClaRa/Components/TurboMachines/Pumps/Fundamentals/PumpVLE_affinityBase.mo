...
//___________________________________________________________________________//
  ...
  ClaRa.Basics.Interfaces.Connected2SimCenter connected2SimCenter(
    ...
    powerAux=(P_shaft - inlet.m_flow*(fluidOut.h - fluidIn.h))) if  contributeToCycleSummary;
...
protected
  SI.EnthalpyMassSpecific h_iso_in(start = bubbleSpecificEnthalpy_pxi(iCom.medium, Delta_p_max, iCom.medium.xi_default)*0.9)
                                                                                                                            "Inlet spec. enthalpy for isentropic state change";
  SI.EnthalpyMassSpecific h_iso_out "Outlet spec. enthalpy for isentropic state change";
  SI.Power P_iso "Power for isentropic pressure increase";
  SI.Power P_shaft "Shaft power";
  ClaRa.Basics.Units.DensityMassSpecific rho_upstream(start = density_phxi(iCom.medium, Delta_p_max, bubbleSpecificEnthalpy_pxi(iCom.medium, Delta_p_max, iCom.medium.xi_default)*0.9, iCom.medium.xi_default)) "Upstream density";
...
protected
  ClaRa.Components.TurboMachines.Fundamentals.GetInputsRotary getInputsRotary
    annotation (Placement(transformation(
        ...
        rotation=-90,
        origin={0,20})));
   TILMedia.VLEFluidObjectFunctions.VLEFluidPointer ptr_iso_in=
   ...
   TILMedia.VLEFluidObjectFunctions.VLEFluidPointer ptr_iso_out=
       TILMedia.VLEFluidObjectFunctions.VLEFluidPointer(
       medium.concatVLEFluidName,
       0,
       medium.mixingRatio_propertyCalculation[1:end - 1]/sum(medium.mixingRatio_propertyCalculation),
...
//_______________Additional media properties _______________
  h_iso_out = noEvent(if iCom.V_flow > 0 then specificEnthalpy_psxi(fluidOut.p, fluidIn.s, fluidOut.xi, ptr_iso_out) else inStream(outlet.h_outflow));
  h_iso_in = noEvent(if iCom.V_flow < 0 then specificEnthalpy_psxi(fluidIn.p, fluidOut.s, fluidIn.xi, ptr_iso_in) else inStream(inlet.h_outflow));
