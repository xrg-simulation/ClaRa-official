...
model Quadratic_FlowFunction "Quadratic | Area and Kd/zeta definition | choked flow | compressible"
  ...
  Modelica.Blocks.Tables.CombiTable1D ValveCharacteristics(table=CL_valve,
      columns={2});
  SI.PressureDifference Delta_p_choke=max(iCom.p_in,iCom.p_out)-max(iCom.p_in,iCom.p_out)*(2/(gamma+1))^(gamma/(max(1e-3,gamma)-1)) "Critical pressure difference, always positive i.e. independent of flow direction";
