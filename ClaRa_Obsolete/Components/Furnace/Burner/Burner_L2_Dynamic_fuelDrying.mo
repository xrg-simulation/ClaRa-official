within ClaRa_Obsolete.Components.Furnace.Burner;
...
extends BaseClasses.CombustionChamberBase(                         redeclare replaceable model Geometry = ClaRa.Basics.ControlVolumes.Fundamentals.Geometry.HollowBlock);
...
public
  Basics.Interfaces.FuelFlueGas_inlet fuelFlueGas_inlet(flueGas(Medium=flueGas), final fuelType=fuelType) annotation (Placement(transformation(extent={{-310,-10},{-290,10}}), iconTransformation(extent={{-310,-10},{-290,10}})));
...
protected
  ...
  TILMedia.Gas.Gas_ph inlet_GasMix(
    p=inlet.flueGas.p,
    xi=xi_flueGasMix_del,
    gasType=flueGas,
    h(start=1.0E4) = h_flueGasMix_del) annotation (Placement(transformation(extent={{-160,-40},{-140,-20}})));
public
...
replaceable model ReactionZone_out =
      ChemicalReactions.CoalReactionZone
    constrainedby ChemicalReactions.PartialReactionZone                          "Model to regard chemical reactions"
...
public
  ...
  Modelica.Units.SI.SpecificHeatCapacity cp_dc_in;
...
protected
  TILMedia.VLEFluid.VLEFluid H2O_props_in(redeclare TILMedia.VLEFluid.Types.TILMedia_SplineWater vleFluidType);
  TILMedia.VLEFluid.VLEFluid H2O_props_out(redeclare TILMedia.VLEFluid.Types.TILMedia_SplineWater vleFluidType);
...
//  0 =outlet.fuel.m_flow + inlet.fuel.m_flow + fuelFlueGas_inlet.fuel.m_flow - m_flow_fuel_burned;
  ...
  n_flow_C = xi_fuel_in[1]*(fuelFlueGas_inlet.fuel.m_flow + inlet.fuel.m_flow)/ClaRa.Basics.Constants.M_C;
  n_flow_H = xi_fuel_in[2]*(fuelFlueGas_inlet.fuel.m_flow + inlet.fuel.m_flow)/ClaRa.Basics.Constants.M_H;
  n_flow_O = xi_fuel_in[3]*(fuelFlueGas_inlet.fuel.m_flow + inlet.fuel.m_flow)/ClaRa.Basics.Constants.M_O;
  n_flow_N = xi_fuel_in[4]*(fuelFlueGas_inlet.fuel.m_flow + inlet.fuel.m_flow)/ClaRa.Basics.Constants.M_N;
  n_flow_S = xi_fuel_in[5]*(fuelFlueGas_inlet.fuel.m_flow + inlet.fuel.m_flow)/ClaRa.Basics.Constants.M_S;
  n_flow_Ash = xi_fuel_in[6]*(fuelFlueGas_inlet.fuel.m_flow + inlet.fuel.m_flow)/ClaRa.Basics.Constants.M_Ash;
  n_flow_H2O = (1 - sum(xi_fuel_in))*(fuelFlueGas_inlet.fuel.m_flow + inlet.fuel.m_flow)/ClaRa.Basics.Constants.M_H2O;

  //_______________/ determination of lambda at burner\_________________________
  n_flow_C_primary = inStream(fuelFlueGas_inlet.fuel.xi_outflow[1])*fuelFlueGas_inlet.fuel.m_flow/ClaRa.Basics.Constants.M_C;
  n_flow_H_primary = inStream(fuelFlueGas_inlet.fuel.xi_outflow[2])*fuelFlueGas_inlet.fuel.m_flow/ClaRa.Basics.Constants.M_H;
  n_flow_O_primary = inStream(fuelFlueGas_inlet.fuel.xi_outflow[3])*fuelFlueGas_inlet.fuel.m_flow/ClaRa.Basics.Constants.M_O;
  n_flow_S_primary = inStream(fuelFlueGas_inlet.fuel.xi_outflow[5])*fuelFlueGas_inlet.fuel.m_flow/ClaRa.Basics.Constants.M_S;

  m_flow_oxygen_req_primary = (n_flow_C_primary + n_flow_H_primary/4.0 + n_flow_S_primary - n_flow_O_primary/2)*ClaRa.Basics.Constants.M_O*2.0;
  ...
  m_flow_oxygen_req = (1 - unburntFraction)*(n_flow_C + n_flow_H/4.0 + n_flow_S - n_flow_O/2)*ClaRa.Basics.Constants.M_O*2.0;
...
//  der(h_flueGas_out) =(Q_flow_wall + Q_flow_top + Q_flow_bottom + inlet.flueGas.m_flow*(flueGasInlet.h - h_flueGas_out) + fuelFlueGas_inlet.flueGas.m_flow*(primaryAir_inlet.h - h_flueGas_out) + inlet.fuel.m_flow*((inlet.fuelType.cp*(inStream(inlet.fuel.T_outflow) - T_0) + Delta_h_f) - h_flueGas_out) + fuelFlueGas_inlet.fuel.m_flow*((fuelFlueGas_inlet.fuelType.cp*(inStream(fuelFlueGas_inlet.fuel.T_outflow) - T_0) + Delta_h_f) - h_flueGas_out) + outlet.fuel.m_flow*((outlet.fuelType.cp*(outlet.fuel.T_outflow - T_0) + Delta_h_f) - h_flueGas_out) + outlet.slag.m_flow*(outlet.slagType.cp*(inStream(outlet.slag.T_outflow) - T_0) - h_flueGas_out) + inlet.slag.m_flow*(inlet.slagType.cp*(inlet.slag.T_outflow - T_0) - h_flueGas_out) + outlet.flueGas.m_flow*(flueGasOutlet.h - h_flueGas_out))/mass;
  ...
  if (1-sum(xi_fuel_in)) <= 1e-9 or (-outlet.fuel.m_flow <= 1e-9) then
    ...
    h_fuel_water_in =TILMedia.VLEFluid.ObjectFunctions.specificEnthalpy_pTxi(
      fuelFlueGas_inlet.fuel.p,
      inStream(fuelFlueGas_inlet.fuel.T_outflow),
      {1},
      H2O_props_in.vleFluidPointer);
    h_fuel_water_out =TILMedia.VLEFluid.ObjectFunctions.specificEnthalpy_pTxi(
      fuelFlueGas_inlet.fuel.p,
      bulk.T,
      {1},
      H2O_props_out.vleFluidPointer);
    Delta_h_fuel_water_evap =TILMedia.VLEFluid.ObjectFunctions.dewSpecificEnthalpy_Txi(
      inStream(fuelFlueGas_inlet.fuel.T_outflow),
      {1},
      H2O_props_in.vleFluidPointer) - TILMedia.VLEFluid.ObjectFunctions.bubbleSpecificEnthalpy_Txi(
      inStream(fuelFlueGas_inlet.fuel.T_outflow),
      {1},
      H2O_props_in.vleFluidPointer);
    if fuelFlueGas_inlet.fuel.LHV_calculationType == "predefined" and inlet.fuel.LHV_calculationType == "predefined" then
    ...
    //cp_out = (cp +  TILMedia.VLEFluidObjectFunctions.specificIsobaricHeatCapacity_pTxi(fuelFlueGas_inlet.fuel.p,bulk.T, {1}, H2O_props_out) * (1-sum(xi_fuel_in)))/sum(xi_fuel_in);
    cp_out =cp - ((1 - sum(xi_fuel_in)) - (1 - sum(xi_fuel_out)))*(
      TILMedia.VLEFluid.ObjectFunctions.specificIsobaricHeatCapacity_pTxi(
      fuelFlueGas_inlet.fuel.p,
      bulk.T,
      {1},
      H2O_props_out.vleFluidPointer) - cp_dc_in);
    cp_dc_in =(cp - (1 - sum(xi_fuel_in))*TILMedia.VLEFluid.ObjectFunctions.specificIsobaricHeatCapacity_pTxi(
      fuelFlueGas_inlet.fuel.p,
      bulk.T,
      {1},
      H2O_props_out.vleFluidPointer))/(sum(xi_fuel_in));
